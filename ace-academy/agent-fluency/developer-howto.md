...
> Practical guide for software developers moving up the Agent Fluency for Builders rubric. Pairs with [../Agent_Fluency_for_Builders_Rubric.md](../Agent_Fluency_for_Builders_Rubric.md) — read the rubric for the levels themselves; this is what to *do* about them.
>
...
- Three sections to grow into: **Practitioner (AF3)**, **Builder (AF4)**, **AF5 fluency** (mastery; rare).
- A fourth section covers **taking on the Agent Fluency Architect role** — a separate org role that AF5 fluency enables but doesn't entail.
- **Practitioner is the expected level by November 2026.** Future hires assessed at hiring; G1/G2 hires trained to Practitioner through the academy program. (See the rubric's *Level expectations*.)
- This guide is for **developers**. For Product Marketing, BizOps, IT, HR/TA, and Legal, see the planned companion *Agent Fluency for Operators*.
- Numbers in this guide ("≥5 PRs", "ten-case eval") are starting points; your tech lead calibrates them per the rubric's *Calibrating measurable bars*.
- For term definitions and external references (AGENTS.md, SKILL.md, MCP, spec-kit, eval frameworks, multi-agent patterns, prompt-injection literature), see the rubric's **Glossary** and **Further reading**.

---

## Reaching Practitioner (AF3)

> **Goal:** roughly 80% of your recurring day-to-day tasks route through an agent — with proper context and verification — and you diligently use any team-shared practices that exist (AGENTS.md, saved skills, prompt cookbook, configured MCPs) rather than running a private setup.
...
- I review agent-generated code with eyes calibrated to *agent* failure modes (made-up imports, plausible-but-wrong APIs, missed edge cases, sycophantic confirmation of my framing), not just human review eyes.
- I catch hallucinated imports, wrong APIs, and off-by-one errors before commit. I can name common failure modes — hallucination, sycophancy, instruction drift, prompt injection, over-confident wrong answers — and watch for them.
- I have a small library of personal Doer skills (saved prompts, personal SKILL.md files) for work that's specific to me. I know which ones might be worth promoting to team-shared.
...
> **Give the agent scoped breadth, not narrow depth.** Agents perform best with four kinds of context, not one: **local context** (the file — imports, surrounding patterns, scope), **goal context** (the failing test, the spec, or the desired outcome), **failure signal** (the actual error message or symptom, verbatim, not paraphrased), and **idiomatic context** (a comparable function from elsewhere in the repo). The "elsewhere" is what most developers skip and what makes the biggest difference. The function you're editing doesn't teach the agent your team's vocabulary — naming conventions, helper-utility patterns, error-handling style, logging idioms. A similar function from another module does. Without idiomatic context, the agent produces generic textbook-quality code; with it, code that matches the codebase. The difference between "right but I have to rewrite half of it" and "right and merges as-is."
...
- Treat agent-generated code review as a distinct skill. Build a personal checklist of what to look for: imports that don't exist, APIs that don't match the version in `package.json` / `requirements.txt`, edge cases the test didn't cover, sycophantic agreement with a wrong framing in your prompt.
...
- **Personal SKILL.md files.** Portable open standard for your reusable assets, supported across 30+ tools.
...
- Reviewing agent-generated code the same way you review human-generated code — missing the agent-specific failure modes.
- Single-turn prompts — you ask once, the answer's off, you give up and write it.
...
- I have at least one scheduled or event-driven agentic workflow running — and I own its operational side (alerts, rollback, on-call).
...
- Encode **"scoped breadth"** into your shared skills. When you author a skill for a recurring task, have it auto-pull 2–3 comparable assets from the repo — similar functions, related test files, relevant ADRs — so the team doesn't have to remember to. The skill should bring the context the team forgets to bring. That's the whole point of skill-as-asset over prompt-as-instruction.
...
- **Splitting.** For large monorepos, an `AGENTS.md` at the root plus per-area `AGENTS.md` files in sub-directories — agents read the nearest one. Don't try to put everything at root.
...
For non-trivial features, agents work much better against a written spec than against a verbal description. The community has converged on a four-stage flow: **constitution → spec → plan → tasks → implement**.
...
- **Implement**: agent executes the tasks, you review each.
...
- Set up at least one autonomous workflow with proper guardrails: nightly dependency-update PRs, on-call summary from yesterday's incidents, weekly PR-review digest. Define rollback, alerting, and cost limits *before* turning it on.
...
- Calibrate autonomy per task deliberately: chat for exploration, approve-each-step for medium risk, run-and-report for low-risk recoverable jobs, fully autonomous only when the blast radius is bounded.
...
AF5 isn't a sprint. It's where you start to teach the patterns, calibrate trust across many contexts, and recognize failure modes the rest of the team won't see for another year. Most strong builders stay at AF4; that's healthy.
...
- Run your own evals against models you don't usually use. Build calibration across the stack, not just the model you default to.
...
> The Architect is a designated org role — not a fluency level. Typically appointed from someone at AF4 or AF5 fluency, with depth as the prerequisite. The role is about scope and authority over a team's agent surface. See the rubric's *Agent Fluency Architect* section for the canonical responsibilities and outcomes.

This section is for developers being considered for, stepping into, or already in the Architect role. If you're not in the role, the AF5 fluency section above is where to focus.
...
- **Time mix.** Expect 30–50% of your week on Architect work: stack decisions, policy writing, peer review, mentoring, postmortem facilitation. The remainder on your own IC work, which is now proof-by-example of the team's standard.
...
- Hoarding the work. Architect ≠ sole builder; if you're authoring everything yourself, you're not stewarding, you're soloing.
- Stack decisions made by preference rather than by eval data. "I like Cursor better" is not a stack rationale.
- Skipping postmortems on agent failures. The team's trust calibration depends on them.
