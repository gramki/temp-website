...
* **Engineering Entities (Run Track as engineering track):**
  * **Run Epic:** A large body of operational engineering work scoped to a single Module (Dim 8). Produces operational System Versions that contribute to Module Package Versions. Triggered by Operational Readiness gaps, Incidents, operational improvement initiatives, or new Module Version readiness. Run Deliberations within Run Epics produce ODRs. (e.g., "Build comprehensive health monitoring for Payments Module").
...
* **Operational Entities:**
  ...
  * **Change Request:** A formal change management envelope governing deployment-related changes, scoped to a Deployment Train (Dim 7) or a specific Station. Three types: Standard, Emergency-Technical (from Incidents), Emergency-Business (from Release Plan acceleration). Successfully completed when all Deployment Tasks AND Verification Tasks pass. (e.g., "CR-2026-0142: Standard, PCI Regulated Train, deploy Payments Module Package Version v2.3.0").
