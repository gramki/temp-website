...
> **Document type:** Exploratory analysis — not a normative specification.
>
...
Reference: [Agent Harness Comparison](agent-harness-comparison.md)
...
This section consolidates all unresolved decisions and open questions from this analysis.

### Control Plane Strategy

1. **Which control plane strategy should Foundry pursue?**
...
3. **Is the Elixir/BEAM ecosystem a fit for Foundry's team capabilities?**
   - Symphony uses Elixir for supervision (polling, retries, isolated workers)
   - Alternative: implement similar supervision in Python/Go

### Agent Adapter Architecture

4. **Which adapter architecture should WO Runtime implement?**
...
5. **Should WO Runtime mandate a minimum interface capability?**
...
7. **How should WO Runtime select interface when multiple are available?**
...
9. **Should fallback preserve interface tier or prioritize availability?**
   ...
   - Capability-degrading: work continues but features may be unavailable

### Ecosystem Dependencies

10. **What is the maintenance risk of depending on Symphony or OpenHands?**
    - OpenAI's commitment to Symphony open-source maintenance needs evaluation
    - OpenHands has broader community contribution model

11. **Should Foundry contribute adapters upstream or maintain forks?**
    - Upstream contribution requires coordination but reduces maintenance
...
- [Agent Harness Comparison](agent-harness-comparison.md)
