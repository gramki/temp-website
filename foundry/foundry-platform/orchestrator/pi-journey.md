...
This document walks through the complete lifecycle of a Product Intent (PI) from creation to release, demonstrating how the Foundry Orchestrator coordinates work across Workspaces.

## Overview

A Product Intent is the Build Track's primary orchestration item. It represents a discrete unit of product evolution—a feature, enhancement, or fix—that flows through the SDLC from idea to customer release.

```
PI Created → Draft Review → Specification → Development + QA Prep → QA Testing → Release
```
