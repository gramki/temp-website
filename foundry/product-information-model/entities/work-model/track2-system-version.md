...
A **sealed, immutable package of Component Versions** — the composed and Component-integration-verified version of a System (Technical). System Version is the second tier of the three-tier versioning model: Component Version (atomic) → **System Version (composed)** → Product Version (complete). Once assembled and verified, a System Version is immutable; only deployment parameters may vary across environments via System Deployment Specifications (Run).
...
> **Build Track ownership for all Systems.** Product-facing Systems and SRE-operational Systems (probes, reconcilers, monitoring agents) alike produce System Versions through the Build Track. The `Purpose / Serving Persona(s)` field on the System entity (Technical) distinguishes who the System serves; the versioning model does not. See DR-036 D10.
...
|---|---|---|
| System | Reference (Technical) | Which System this version belongs to |
...
| Integration Epic | Reference (Build) | Integration Epic that produced this verification (if applicable) |
...
|---|---|---|
| Belongs to | System (Technical) | System Version is a versioned, integration-verified instance of a System |
| Composes | Component Version(s) (Build) | System Version seals specific Component Versions in its BOM |
| Composed into | Product Version (Build) | System Versions compose a Product Version |
| Described by | System Deployment Specification(s) (Run) | Environment-specific deployment is specified by System Deployment Specifications |
| Deployed by | Deployment Task (Run) | System Versions are deployed via System Deployment Specifications applied by Deployment Tasks |
| Produced by | Integration Epic / Integration Story (Build) | Integration work produces contracts and tests that verify the composition |
| Reflects in | Operational Readiness (Operational) | Released System Versions feed Operational Readiness assessment per environment |
