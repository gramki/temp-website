...
**System/Component scope, not Module scope:** Technical Tasks speak the System language ("Implement gRPC endpoint in fx-service," "Add Kafka consumer in payments-service"). They are the bridge from functional intent (Stories, Module-scoped) to technical implementation (Systems, Technical). A single Story may spawn Technical Tasks in multiple Systems because the Module-to-System mapping is many-to-many. See DR-026.
...
| Product Intent | Reference (Strategy) | Parent Product Intent that authorizes this work |
| Story | Reference (Build) | Parent Story (or Integration Story) this Task implements |
| System | Reference (Technical) | Which System this Task is implemented in |
| Component | Reference (Technical) | Which Component within the System (optional, for complex Systems) |
...
|---|---|---|
| Implements | Story (Build) | Technical Task implements a Story |
| Implements | Integration Story (Build) | Technical Task implements an Integration Story |
| Authorized by | Product Intent (Strategy) | Technical Task is subordinate to Product Intent orchestration |
| Scoped to | System (Technical) | Task is implemented within a specific System |
| Scoped to | Component (Technical) | Task may target a specific Component within the System (optional) |
| Included in | System Version (Build) | Completed Task is included in the next System Version |
| Belongs to | Epic (Build) | Task belongs to an Epic (via Story) |
