...
2. **Corrective actions** — follow-up work items routed to the appropriate track: Bugs (Track 2), Run Epics (Track 3), Signals (Track 1), ODRs (Dim 7), Evolve Findings (Track 5), Maintenance Tasks (Track 3)
...
|---|---|---|
| Incident(s) | List of References (Track 3) | Incident artifact(s) being reviewed — may batch related incidents (e.g., parent + children) |
| Incident Response Task(s) | List of References (Track 3) | The response task(s) for context on what was done |
...
| Bug (Track 2) | Work entity (follow-up) | Product defect identified as root cause; provenance: Run | Build Track |
| Signal — Problem (Dim 1) | Definition Model update | Systemic product issue surfaced for Discovery | Discovery Track |
| Run Epic (Track 3) | Work entity (follow-up) | Operational tooling gap identified (e.g., missing probes, insufficient automation) | Run Track |
| ODR (Dim 7) | Definition Model update | Operational decision driven by incident findings (e.g., deployment strategy change, monitoring architecture revision) | Definition Model |
| Evolve Finding (Track 5) | Work artifact (follow-up) | Process gap identified (e.g., "runbook was outdated", "escalation policy unclear") | Evolve Track |
| Maintenance Task (Track 3) | Work entity (follow-up) | Corrective maintenance needed (e.g., "patch library X across all environments") | Run Track |

## Relationships

| Direction | Related Entity | Relationship |
|---|---|---|
| Reviews | Incident(s) (Track 3) | PIR reviews one or more Incident artifacts |
| Reviews | Incident Response Task(s) (Track 3) | PIR reviews the response effectiveness |
| Produces | Post-Incident Report (artifact) | Primary output: durable assessment artifact |
| May produce | Bug (Track 2) | Root cause is a product defect |
| May produce | Signal — Problem (Dim 1) | Systemic issue surfaced for Discovery |
| May trigger | Run Epic (Track 3) | Operational tooling gap requires engineering investment |
| May produce | ODR (Dim 7) | Operational decision required |
| May produce | Evolve Finding (Track 5) | Process gap identified |
| May produce | Maintenance Task (Track 3) | Corrective maintenance needed |
| May update | Operational Readiness (Dim 7) | PIR findings may downgrade readiness posture for affected System x Environment |
| May update | Operational Pain (Dim 7) | PIR may surface or provide evidence for Operational Pains |
| Informs | Deployment Planning Task (Track 3) | PIR findings inform deployment risk assessment |

## Examples

| PIR | Incident(s) | Root Cause | Corrective Actions | Status |
|---|---|---|---|---|
| "PIR: Database cluster failover — 2026-02-15" | INC-2026-0849, INC-2026-0847, INC-2026-0848 | "Cloud provider AZ failure triggered failover; application connection pool did not reconnect within timeout" | Bug: "Connection pool reconnect logic" (Track 2); Run Epic: "Automated AZ failover validation" (Track 3); ODR: "Multi-AZ deployment requirement for all stateful services" (Dim 7) | Complete |
| "PIR: Post-deployment error rate — Payments LATAM" | INC-2026-0850 | "Database migration script did not handle existing data format for LATAM-specific payment types" | Bug: "Migration script LATAM data handling" (Track 2); Evolve Finding: "Deployment Planning checklist should include LATAM data format validation" (Track 5) | Complete |
| "PIR: FX API latency spike" | INC-2026-0847 | "Cache eviction under load caused thundering herd to upstream FX provider" | Bug: "Cache stampede protection" (Track 2); Run Epic: "FX cache warming automation" (Track 3) | Scheduled |
