...
An API Module is structurally a Structural Module — it participates in the Product → Module → Capability → Feature hierarchy, has a bounded context, archetype classification, Technical internals, and Build Track versioning. What makes it a Ecosystem module is: (1) its purpose is externally-facing extensibility, (2) its capabilities are often compositional — curating capabilities from other Structural modules for a use-case-oriented surface, and (3) it carries Ecosystem-specific concerns (Developer Personas, Programmatic Personas, API contracts, SLOs).
...
| Composes From | Reference(s) | Structural Module(s) whose capabilities this module curates |
...
|---|---|---|
| Is a | Module (Structural) | Structurally a Structural Module with Ecosystem concerns |
| Composes from | Module(s) (Structural) | Curates capabilities from underlying modules |
| Serves | Developer Persona (Ecosystem) | Designed for these developers |
| Serves | Programmatic User Persona (Ecosystem) | Consumed by these systems at runtime |
| Contains | API Operation(s) (Ecosystem) | Named contractual operations within this module |
| Governed by | API Compatibility Contract (Ecosystem) | Versioning and stability commitments |
| Wrapped by | SDK/Library Module (Ecosystem) | Language-specific clients wrap this surface |
| Bridged by | Integration Module (Ecosystem) | Connectors may use this module's operations |
| Assessed by | Win Review (Win) | API adoption and health reviewed |
| Monitored by | System Monitoring (Run) | Runtime performance monitored |
