...
A role archetype that uses the product's Human-Interactive Modules. A User Persona represents a distinct type of human who interacts with the product — their context, goals, frustrations, technical proficiency, and jobs to be done. User Personas are *users* of the product; they are distinct from Buying Personas (Dim 3), who are *purchasers* of the product.
...
**Role Basis:** Personas are defined by **product role** (what they do in the product — "Approver," "Analyst," "Admin") or **business domain role** (what they do in their organization — "AP Clerk," "Compliance Officer," "Treasury Analyst"). Both are valid bases. Product role personas emphasize the functional relationship with the product; business domain role personas emphasize the real-world context and jobs. Most personas combine both: "AP Clerk" is a business domain role, but their product role is "Payment Initiator." The product's RBAC (role-based access control) mapping — which features and journeys are available to which product roles — is a Dim 8 / implementation concern, not a Dim 4 entity.
...
- Pain (Dim 3) has no one to endure it — Pain is "endured by User Persona, cared about by Buying Persona"
...
**Distinction from Buying Persona (Dim 3):** A Buying Persona decides to purchase; a User Persona uses the product. They may overlap (a CFO who both approves budget and uses the executive dashboard) but often diverge (the CFO approves; the AP Clerk uses daily). User Personas endure Pains; Buying Personas *care about* those Pains.
...
| Customer Segment(s) | List of References (Dim 3) | Which segments this persona exists in (an AP Clerk at LATAM Enterprise vs. US Mid-Market may differ) |
...
| Jobs (JTBD) | List of References (Dim 4) | The Jobs this persona needs to accomplish |
| Preferred Channel(s) | List of References (Dim 4) | Which UX Channel(s) this persona prefers or primarily uses. Influences channel investment priority and journey design. |
...
|---|---|---|
| Has | Job (Dim 4) | Persona has Jobs to be done |
| Follows | User Journey (Dim 4) | Persona follows Journeys to accomplish Jobs |
| Prefers | UX Channel (Dim 4) | Persona prefers specific Channel(s) |
| Endures | Pain (Dim 3) | Persona endures Pains (which Buying Personas care about) |
| Belongs to | Customer Segment (Dim 3) | Persona exists within Customer Segment(s) |
| Work Model | Research Task (Track 1) | Research Tasks validate Persona hypotheses |
| Work Model | Modeling Task (Track 1) | Modeling Tasks formalize Personas in Dim 4 |
