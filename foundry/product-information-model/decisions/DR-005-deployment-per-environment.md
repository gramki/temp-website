...
The model needed to represent how System Versions move from "quality-gated artifact" to "running in production." Initially, "Deployed" was considered as a status on the System Version entity (e.g., `Building → Released → Deployed`). However, this was challenged:
...
Model **Deployment** as a standalone Run Track entity with per-environment tracking, rather than as a status on System Version.

System Version status lifecycle: `Building → Released` (only two states).
...
1. **Reality is per-environment.** A System Version may be deployed to production-us on Tuesday and production-eu on Thursday. Modeling "Deployed" as a version status cannot represent this.
...
3. **Decouples release from deployment.** A System Version can be `Released` (quality gates passed) without being deployed anywhere. Once deployed (even to production), it may still be behind a feature flag — not yet part of an activated Customer Release. This supports dark launches and canary strategies.
...
- **Positive:** Clean separation: System Version status = quality gate, Deployment status = operational state.
