...
DR-035 redefined Technical: **Component** is the individually buildable deployable artifact; **System** is the operational deployment grouping of Components, versioned and deployed as a whole by SRE/ops. That amendment exposed a structural mismatch in the versioning and deployment model established by DR-026 through DR-029:

1. **Misaligned versioning tiers.** DR-026 introduced three tiers — System Version (atomic) → Module Version (integration-verified composition) → Product Version — when "System" meant the atomic deployable. Under DR-035, the atomic tier is **Component Version**; **System Version** must mean a sealed BOM of Component Versions for a Technical System.
...
3. **Operator-facing package layers.** DR-027 and DR-029 introduced Module Package / Product Package (Operational specifications + Run Track versions) and MDD/PDD as separate operator-facing composition layers atop Module Version. SRE operational Systems are legitimate Technical Systems (DR-035, DR-027); they belong in the same Build Track versioning chain as product Systems, not in a parallel Package entity.
...
5. **Missing Product-level composition spec.** No Definition Model entity declared which Systems constitute a Product. Module boundaries (Structural) do not define deployment topology — a Product may span many Systems across many Modules with equal structural standing.

6. **Operational scope creep.** Module Package and Product Package entities placed composition templates in Operational alongside runtime operational entities (Deployment Environment, Deployment Train, Station). Operational should retain runtime and governance concerns only.
...
**Decision:** **Component Version** is the versioned, quality-gated output of building a single Component (Technical) — the atomic build artifact (container image, Lambda package, frontend bundle, etc.). It replaces the semantic role previously assigned to "System Version" in DR-026 (when System meant atomic deployable).
...
### D4: Product Specification is a new Technical entity

**Decision:** Introduce **Product Specification** as a Definition Model entity in Technical. It has a 1:1 relationship with Product (Structural) and declares all Systems that constitute the Product — product-serving and SRE operational Systems alike.
...
- Module Package (Operational specification)
- Module Package Version (Work Model artifact)
- Product Package (Operational specification)
...
**Rationale:** Package entities duplicated composition already expressible as Systems in Technical. Operator-facing probes, reconcilers, and dashboards are Systems with operational Purpose — they receive Component and System Versions like any other System, not a parallel Package layer.
...
**Rationale:** DR-027 correctly identified operational Systems as legitimate Technical Systems with code and CI/CD. The error was routing their versions through a parallel Package artifact. One Build Track, one versioning chain, preserves traceability from deployment back to build artifacts.
...
**Decision:** Capability availability in a deployed Product is determined by tracing: **Capability** (Structural) → realizing **System(s)** (Technical) → **System Version** in the Product Version BOM → deployed via System Deployment Specification.
...
### D13: Operational retains runtime concerns only; no Package entities

**Decision:** Operational retains runtime, environment, and governance entities (Deployment Environment, Deployment Train, Station, Cluster, Host, etc.). No Package specification entities remain in Operational.

**Rationale:** Separating composition templates (now Product Specification in Technical) from runtime topology (Operational) restores a clean Definition Model boundary. Operational answers "where and how governed execution happens," not "what Systems constitute the product."
...
**Why Product Specification in Technical?**
Product Specification declares Systems — Technical entities. It is architect-authored (alongside PSD technical mapping per DR-035) and stable across Product Versions. Placing it in Structural would conflate commercial product definition with technical System topology.
...
**Positive:**
- Versioning chain aligns with DR-035 Technical semantics: Component Version → System Version → Product Version
...
- Operational scope is narrowed to runtime and governance; composition moves to Technical
...
- Operational Module Package / Product Package entity files → removed; Product Specification added to Technical
...
| DR-029 | D1–D3 (Module/Product Package spec and versions in Operational / Run Track) superseded by D4, D7, D13; Change Request and deployment workflow decisions amended by D11 to System Deployment / Product Deployment scope |
