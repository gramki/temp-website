# DR-007: Restructure Customer Value
...
Restructure Customer Value (the dimension addressing "Why Buy") with six entities organized around the complete buying decision:

1. **Customer Segment** — the organizing entity anchoring all other Customer Value entities.
...
- **Separation of concerns between promise types:** Value Propositions map to Structural entities. Service Commitments map to Operational entities. Compliance Posture influences both structure and operations. Mixing them in a single entity would obscure these distinct relationships.
...
- Adoption Barriers surface as Signals (Strategy) for discovery prioritization
- Clean separation between value promise, reliability promise, and compliance promise
- Modeling Tasks (Discovery) can now target specific Customer Value entities

### Negative
- Six entities in Customer Value (up from three) — increased model surface
- Customer Promise subtype structure may require explanation for stakeholders unfamiliar with the distinction
- Overlap between Customer Value (customer-facing commitment) and Operational (operational implementation) for Service Commitments must be managed explicitly
