...
* **System:** The operational deployment grouping of Components — what SRE versions and deploys as a whole. When SRE deploys "Payments System v3.1," they deploy a composed release of all its Components together. The technical counterpart to Module (Dim 8): where Module captures the functional boundary ("Payments Module"), System captures the operational deployment boundary ("payments-system"). The mapping is many-to-many: a Module may be implemented by multiple Systems; a shared System may serve multiple Modules. *(Amended from DR-024 by DR-035.)*
...
* **Component:** The atomic deployable artifact within a System — a container image, Lambda package, frontend bundle, or similar independently buildable unit. Components are independently buildable (each has its own build artifact and CI pipeline) but are not independently deployed to production; they are deployed as part of their parent System's composed release. Has a **Component Archetype** enum classifying the artifact type (API Service, Web Application, Event-Driven Worker, Batch Job, Data Store, Integration Adapter, Gateway, CLI/SDK — see `draft-archetypes.md`). *(Amended from DR-024 by DR-035.)*
